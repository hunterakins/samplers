...
class ParallelTemperedAdaptiveSampler:
    ...
    def sample(self, x0_list, sigma0_list, chain_params_list, N, swap_interval=1):
        ...
        chain_list = self._init_chains(x0_list, sigma0_list,chain_params_list, N)
        ...
        for chain in chain_list:
            ...
            chain.log_probs[0] = log_p
            chain.acceptance_ratios[0] = 1

    
        # first run for a bit to find a good spot...
        N_burn = int(N/10)
        for k in range(1, N_burn): 
            for chain_ind in range(num_chains):
                ...
                # add sample to chain
                chain.samples[:, 0] = x_curr.copy()
                chain.log_probs[0] = log_p_curr
                chain.acceptance_ratios[0] = num_accepted_list[chain_ind] / (1)  # acc. ratio
        ...
        for i in range(1, N): 
            ...
            for chain_ind in range(num_chains):
                ...
                # add sample to chain
                chain.samples[:, i] = x_curr.copy()
                chain.log_probs[i] = log_p_curr
                chain.acceptance_ratios[i] = num_accepted_list[chain_ind] / (i)  # acc. ratio
                ...
                if i == chain_params.nu:
                    ...
                    chain.mu = mu
                if i > chain_params.nu:
                    if chain_params.update_after_burn:
            ...
            if (i%swap_interval==0) and (i > chain_params.nu): 
                # ........
                X = i / swap_interval
                even =  X% 2
                even = not even
                for chain_ind in range(num_chains-1):
                    ...
                    if ((even) and (hot_chain_ind % 2 == 0)) or ((not even) and (hot_chain_ind % 2 == 1)):
                        ...
                        if u < omega: # swap
                            ...
                            # update the chain samples and log_p
                            cold_chain.samples[:, i] = x_curr_list[cold_chain_ind].copy()
                            cold_chain.log_probs[i] = curr_log_p_list[cold_chain_ind]
                            hot_chain.samples[:, i] = x_curr_list[hot_chain_ind].copy()
                            hot_chain.log_probs[i] = curr_log_p_list[hot_chain_ind]

                            # note it in the swap mat
                            t1, t2 = swap_mat[hot_chain_ind, i], swap_mat[cold_chain_ind, i]
                            swap_mat[hot_chain_ind, i] = t2
                            swap_mat[cold_chain_ind, i] = t1
