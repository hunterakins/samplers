...
def mh_walk(x0, N_tune, f_proposal, prop_cov, f_log_prior, f_log_lh, beta):
    ...
    return x_samples, acceptance_ratios
...
class AdaptiveTransDPTSampler:
        ...
        for i in range(num_chains):
            ...
            for j in range(num_dims):
                ...
                for var_i, var in enumerate(variance_range): # now generate 100 samples for each variance
                    ...
                    x_samples, acc_ratios = mh_walk(x, N_tune, self.f_proposal, prop_cov*sd, self.f_log_prior, self.f_log_lh, beta)
                ...
                x_samples, acc_ratios = mh_walk(x, N_scm, self.f_proposal, prop_cov*sd, self.f_log_prior, self.f_log_lh, beta)
                #print(acc_ratios[-1])
    ...
    def initialize_chains(self, N, N_burn_in, nu, f_prior, update_after_burn, swap_interval, prop_covs):
        ...
        self.swap_mat = np.zeros((len(self.beta_arr), N+1))
    ...
    def _birth_move(self, i, j):
        ...
        move_p_ratio = self.move_probs[dim_ind][0]/self.move_probs[dim_ind+1][1] # birth/death
        u, log_gu = self.f_proposal(self.birth_sigma)
        xprime = x.copy()
        ...
        alpha = (log_p + 0) - (curr_log_p + log_gu) + np.log(move_p_ratio) # 
        alpha = np.exp(alpha)
    ...
    def _death_move(self, i, j):
        ...
        log_prior = self.f_log_prior(xprime)
        log_lh = self.f_log_lh(xprime)
        sigma = self.birth_sigma
        log_guprime = self.f_log_gprime(u, **{'sigma':sigma})
        log_p = log_prior + beta*log_lh
        alpha = (log_p + log_guprime) - (curr_log_p + 0) + np.log(move_p_ratio) #
        alpha = np.exp(alpha)
        if np.random.rand() < alpha: #accept move
    ...
    def _swap_chains(self, j):
        ...
        # swaps (DEO)
        self.swap_mat[:,j+1] = self.swap_mat[:,j]
            ...
            for chain_ind in range(num_chains-1):
                ...
                if ((even) and (hot_chain_ind % 2 == 0)) or ((not even) and (hot_chain_ind % 2 == 1)):
                    ...
                    if u < omega: # swap
                        ...
                        # fetch log prios
                        cold_log_prior = cold_chain.curr_log_prior.copy()
                        hot_log_prior = hot_chain.curr_log_prior.copy()
                        # swap priors
                        cold_chain.curr_log_prior = hot_log_prior
                        hot_chain.curr_log_prior = cold_log_prior

                        # swap log posteriors
                        hot_chain.curr_log_p = beta_hot*cold_lh + cold_log_prior
                        cold_chain.curr_log_p = beta_cold*hot_lh + hot_log_prior

                        # swap samples
                        t1 = cold_chain.samples[:, j+1].copy()
                        t2 = hot_chain.samples[:,j+1].copy()
                        cold_chain.samples[:,j+1] = t2
                        hot_chain.samples[:,j+1] = t1
    ...
    def cold_chain_diagnostic_plot(self, density=False):
        """
        Only for the cold chain...
        """
        chain = self.chain_list[0]
        samples = chain.samples
        log_probs = chain.log_probs
        acceptance_ratios = chain.acceptance_ratios
        birth_acceptance_ratios = chain.birth_acceptance_ratios
        ...
        log_p_ar_fig.suptitle('Log posterior and acceptance ratio for chain with $\\beta = {0}, T = {1} $'.format(round(chain.params.beta, 2), round(1/chain.params.beta, 2)))
        axes[1].plot(acceptance_ratios, label='pert acceptance')
        axes[1].plot(death_acceptance_ratios, label='death acceptance')
        axes[1].plot(birth_acceptance_ratios, label='birth acceptance')
        axes[1].legend()
